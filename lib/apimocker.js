...
apiMocker.setRoutes = function(webServices) {
  ...
  _.each(topLevelKeys, function(key) {
    ...
    _.each(svc.verbs, function(v) {
      ...
      if (typeof apiMocker.callsQty[key] === "undefined") {
        apiMocker.callsQty[key] = {}
      }
      apiMocker.callsQty[key][v] = 0;
    });
  });
};
...
apiMocker.sendResponse = function(req, res, serviceKeys) {
  ...
  setTimeout(function() {
    ...
    if (typeof options.mocks !== "undefined" 
      && typeof options.mocks[apiMocker.callsQty[options.serviceUrl][options.verb]] !== "undefined" ){
      mockFile = options.mocks[apiMocker.callsQty[options.serviceUrl][options.verb]];
    }
    mockPath = path.join(apiMocker.options.mockDirectory, mockFile);
    ...
    apiMocker.callsQty[options.serviceUrl][options.verb]++;
    
    if ( typeof options.mocks !== "undefined" && _.isArray(options.mocks)
      && apiMocker.callsQty[options.serviceUrl][options.verb] >= options.mocks.length){
      apiMocker.callsQty[options.serviceUrl][options.verb] = 0;
    }
    ...
    if (options.contentType) {
      ...
      res.status(options.httpStatus).sendfile(encodeURIComponent(mockFile), {root: apiMocker.options.mockDirectory});
    }
